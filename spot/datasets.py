...
class SimpleDataset(Dataset):
    def __init__(self, dir_path, image_size=224):
        self.dir_path = dir_path
        self.image_files = [f for f in os.listdir(dir_path) if f.endswith('.jpg' or '.png')]
        self.transform = transforms.Compose([transforms.Resize(size = image_size, interpolation=transforms.InterpolationMode.BILINEAR),
                               transforms.CenterCrop(size = image_size),
                               transforms.ToTensor(),
                               transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))])
        
        self.transform_no_norm = transforms.Compose([transforms.Resize(size = image_size, interpolation=transforms.InterpolationMode.BILINEAR),
                               transforms.CenterCrop(size = image_size),
                               transforms.ToTensor()])
    ...
    def __getitem__(self, idx):
        ...
        img_orig = self.transform_no_norm(image)
