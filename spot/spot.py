...
class SPOT(nn.Module):
    ...
    def pca_visualization(self, embeddings: torch.Tensor, filename) -> torch.Tensor:
        """
        Perform PCA on image embeddings and process them for visualization.
        
        Args:
            embeddings (torch.Tensor): Tensor of shape (num_patches, depth)
        
        Returns:
            torch.Tensor: RGB image tensor of shape (3, 224, 224)
        """
        num_patches, depth = embeddings.shape
        
        # Determine the spatial dimensions (assuming square image patches)
        h = w = int(math.sqrt(num_patches))
        assert h * w == num_patches, "num_patches must be a perfect square"
        
        # Perform PCA to reduce dimensionality to 3
        pca = PCA(n_components=3)
        reduced = pca.fit_transform(embeddings.cpu().numpy())  # Convert to NumPy for PCA
        reduced = torch.tensor(reduced, dtype=torch.float32)  # Convert back to Torch
        
        # Reshape to (h, w, 3)
        image = reduced.view(h, w, 3)
        
        # Normalize to [0, 1] for visualization
        image = (image - image.min()) / (image.max() - image.min())
        
        # Convert to (3, h, w) for resizing
        ...
        # Convert to (224, 224, 3) for plotting
    ...
    def forward(self, image):
        ...
        return loss_mse, slots_attns, dec_slots_attns, slots, dec_recon, attn_logits, emb_input
