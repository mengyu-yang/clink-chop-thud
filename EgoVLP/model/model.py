...
from EgoVLP.model.video_transformer import SpaceTimeTransformer
...
class FrozenInTime(BaseModel):
    def __init__(self,
        ...
        self.video_params = {
            ...
            'time_init': video_time_init,
            'attention_style': video_attention_style,
        }
        ...
        if self.text_params['model'].startswith('distilbert'):
            ...
            self.text_model = AutoModel.from_pretrained(self.text_params['model'])
        self.text_model.train()

        if self.video_params['model'] == "SpaceTimeTransformer":
            num_frames = self.video_params.get('num_frames', 4)
            time_init = self.video_params.get('time_init', 'zeros')
            attention_style = self.video_params.get('attention_style', 'frozen-in-time')
            arch_config = self.video_params.get('arch_config', 'base_patch16_224')
            vit_init = self.video_params.get('vit_init', 'imagenet-21k')
            ...
            if arch_config == 'base_patch16_224':
                ...
                vit_model = torch.load("/coc/flash5/myang415/objects_project/EgoVLP/pretrained/egovlp.pth")
            ...
            raise NotImplementedError(f"{self.video_params['model']} not implemented")
    ...
    def forward(self, data, use_object_tokens):

        text_data = data['text']
        video_data = data['video']
        obj_mask = data['object_masks']
        obj_idx_mask = data['object_idx_masks']

        text_embeddings = self.compute_text(text_data)  # (B, D)
        text_tokens = self.compute_text_tokens(text_data)  # (B, N, D)
        video_embeddings = self.compute_video(video_data, obj_mask, obj_idx_mask, use_object_tokens)  # (B, 1+P, D)

        return text_embeddings, text_tokens, video_embeddings
    ...
    def compute_video(self, video_data, obj_mask, obj_idx_mask, use_object_tokens):
        video_embeddings = self.video_model(video_data, obj_mask, obj_idx_mask, use_object_tokens)
