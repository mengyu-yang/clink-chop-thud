...
class PatchifyMask(nn.Module):
    """ Video to Patch Embedding
    """

    def __init__(self, img_size=224, patch_size=16, in_chans=1, num_frames=8, obj_threshold=0.0, device='cuda'):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        num_patches = (img_size[1] // patch_size[1]) * (img_size[0] // patch_size[0]) * num_frames
...
class VarAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        self.obj_qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
    ...
    def forward(self, x, einops_from, einops_to, obj_embeds=None, num_objects=None, num_frames=None, **einops_dims):
        h = self.num_heads
        # project x to q, k, v vaalues
        q, k, v = self.qkv(x).chunk(3, dim=-1)
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), (q, k, v))
        ...
        # Project object embeddings to q, k, v values
        if obj_embeds is not None:
            obj_q, obj_k, obj_v = self.obj_qkv(obj_embeds).chunk(3, dim=-1)
            obj_q, obj_k, obj_v = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), (obj_q, obj_k, obj_v))

            obj_q *= self.scale

            # Reaarange object embeddings across time or space
            obj_q, obj_k, obj_v = map(lambda t: rearrange(t, f'{einops_from} -> {einops_to}', f=num_frames, n=num_objects), (obj_q, obj_k, obj_v))
        ...
        # Cross attention between object embeddings and patch embeddings
        if obj_embeds is not None:
            out = attn(obj_q, k_, v_)
        else:
            out = attn(q_, k_, v_)
...
class SpaceTimeBlock(nn.Module):
    ...
    def forward(self, x, einops_from_space, einops_to_space, einops_from_time, einops_to_time,
                time_n, space_f, obj_embeds=None):

        if obj_embeds is not None:
            num_objects = obj_embeds.shape[1] // space_f
        else:
            num_objects = None
        ...
        space_output = self.attn(self.norm1(time_residual), einops_from_space,
                                 einops_to_space, obj_embeds=obj_embeds, num_frames=space_f, num_objects=num_objects, f=space_f)
        
        if self.attention_style == 'frozen-in-time':
            if obj_embeds is not None:
                # Remove cls token from space_output
                space_output = space_output[:, 1:]
                space_residual = obj_embeds + self.drop_path(space_output)
            else:
        ...
        if obj_embeds is not None:
            obj_embeds = obj_embeds + self.drop_path(self.mlp(self.norm2(space_residual)))
            return x, obj_embeds
        else:
...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.mask_patch = PatchifyMask(
            img_size=img_size, patch_size=patch_size, in_chans=1, num_frames=num_frames, device='cuda')
        
        self.obj_embed = nn.Conv1d(embed_dim, embed_dim, 1)
    ...
    def forward_features(self, x):
        b, curr_frames, channels, _, _ = x.shape  # (B, T, C, H, W)
        x = self.patch_embed(x)  # (B*T, D, H_patchified, W_patchified)
        x = x.flatten(2).transpose(2, 1)
        x = x.reshape(b, -1, self.patch_embed.embed_dim)  # (B, N, D) where N = num_patches * num_frames
    ...
    def forward_features_obj_tokens(self, x, obj_mask, obj_idx_mask):   
        b, curr_frames, channels, _, _ = x.shape  # (B, T, C, H, W)
        x = self.patch_embed(x)  # (B*T, D, H_patchified, W_patchified)
        x = rearrange(x, '(b t) d h w -> b (t h w) d', b=b, t=curr_frames)  # (B, N, D)
        ...
        _, _, num_objects, _, _ = obj_mask.shape
        mask_patchified = self.mask_patch(obj_mask)  # (B*T*O, 1, H_patchified, W_patchified)
        mask_patchified = rearrange(mask_patchified, '(b t o) 1 h w -> b (t h w) o 1', b=b, t=curr_frames, o=num_objects)

        # Separate out cls embedding from patch embeddings
        cls_embed, patch_embed = x[:, 0], x[:, 1:]
        
        # Patch-wise multiply the object masks with the patch embeddings so only the object embeddings are non-zero
        obj_embeds = mask_patchified * patch_embed.unsqueeze(-2)  # (B, N, O, 1) * (B, N, 1, D) = (B, N, O, D)
        obj_embeds = rearrange(obj_embeds, 'b n o d -> (b o) d n')
        obj_embeds = self.obj_embed(obj_embeds)
        obj_embeds = rearrange(obj_embeds, '(b o) d (t h w) -> b o t (h w) d', o=num_objects, t=curr_frames, h=int(math.sqrt(n)), w=int(math.sqrt(n)))

        # Maxpool to get single embedding per object per frame
        obj_embeds = obj_embeds.max(dim=-2)[0]
        obj_embeds = rearrange(obj_embeds, 'b o t d -> b (t o) d', o=num_objects)

        for blk in self.blocks:
            x, obj_embeds = blk(x, self.einops_from_space, self.einops_to_space, self.einops_from_time,
                    self.einops_to_time,
                    time_n=n, space_f=f,
        ...
        # x = self.norm(x)
        # x = self.pre_logits(x)
        obj_embeds = self.norm(obj_embeds)
        obj_embeds = self.pre_logits(obj_embeds)

        return obj_embeds

    def forward(self, x, obj_mask, obj_idx_mask, use_object_tokens):
        if use_object_tokens:
            x = self.forward_features_obj_tokens(x, obj_mask, obj_idx_mask)
        else:
